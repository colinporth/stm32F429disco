...
//}}}
//{{{
void cLcd::rect1 (uint16_t colour, int16_t x, int16_t y, uint16_t width, uint16_t height) {
...
//}}}
//{{{
void cLcd::stamp1 (uint16_t colour, uint8_t* src, int16_t x, int16_t y, uint16_t width, uint16_t height) {
...
void cLcd::endRender (bool forceInfo) {
  ...
  if ((mShowTitle || forceInfo) && !mTitle.empty()) {
    //{{{  draw title
    text (COL_YELLOW, cWidget::getFontHeight(), mTitle, 0, y, getWidthPix(), cWidget::getBoxHeight());
  ...
  if (mShowInfo || forceInfo) {
    ...
    for (auto lineIndex = (int)mFirstLine; lineIndex <= lastLine; lineIndex++) {
      ...
      auto xinc = text (COL_GREEN, cWidget::getFontHeight(),
                        ...
                        x, y, getWidthPix(), cWidget::getBoxHeight());
      x += xinc + 3;
  ...
  if (mShowFooter || forceInfo)
    ...
    text (COL_WHITE, cWidget::getFontHeight(),
          dec (xPortGetFreeHeapSize()) + ":free " +
          dec (xPortGetMinimumEverFreeHeapSize()) + ":lo " +
          //dec (osGetCPUUsage()) + "% " +
         dec (mDrawTime) + "ms ", 0, -cWidget::getFontHeight() + getHeightPix(), getWidthPix(), cWidget::getFontHeight());
    //}}}
  ready();
  ...
  if ((pressCount > 30) && (x <= mStringPos) && (y <= cWidget::getBoxHeight()))
    ...
    if (x <= mStringPos) {
      ...
      if (y < 2 * cWidget::getBoxHeight())
        displayTop();
      else if (y > getHeightPix() - 2 * cWidget::getBoxHeight())
        displayTail();
      }
    }
  else {
...
void cLcd::updateNumDrawLines() {
  ...
  auto numDrawLines = getHeightPix() / cWidget::getBoxHeight();
